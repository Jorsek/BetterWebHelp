...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<!-- "no" and "yes" are valid values; non-'no' is ignored -->
	...
	<xsl:template match="*[contains(@class,' topic/related-links ')]" mode="breadcrumb">
		<xsl:for-each
			select="descendant-or-self::*[contains(@class,' topic/related-links ') or contains(@class,' topic/linkpool ')][child::*[@role='ancestor']]">
			...
			<div class="breadcrumb">
				<xsl:if test="contains($include.roles, ' previous ')">
					<xsl:choose>
						...
						<xsl:when test="*[@href][@role='previous']">
							<xsl:apply-templates select="*[@href][@role='previous'][1]"
								mode="breadcrumb"/>
						</xsl:when>
						<xsl:otherwise/>
					</xsl:choose>
				...
				<xsl:if
					test="contains($include.roles, ' next ') and contains($include.roles, ' next ')">
				...
				<xsl:if test="contains($include.roles, ' next ')">
					<xsl:choose>
						...
						<xsl:when test="*[@href][@role='next']">
							<xsl:apply-templates select="*[@href][@role='next'][1]"
								mode="breadcrumb"/>
						</xsl:when>
						<xsl:otherwise/>
					</xsl:choose>
				...
				<xsl:if
					test="contains($include.roles, ' next ') and contains($include.roles, ' next ') and contains($include.roles, ' ancestor ')">
					<!--if we have either next or previous, plus ancestors, separate the next/prev from the ancestors with a vertical bar-->
					<xsl:if
						test="(*[@href][@role='next'] or *[@href][@role='previous']) and *[@href][@role='ancestor']">
						<xsl:text> | </xsl:text>
					</xsl:if>
				</xsl:if>
	...
	<xsl:template match="*[contains(@class,' topic/related-links ')]" mode="prereqs">
		...
		<xsl:if
			test="descendant::*[contains(@class, ' topic/link ')][not(ancestor::*[contains(@class, ' topic/linklist ')])][@importance='required' and (not(@role) or @role='sibling' or @role='friend' or @role='previous' or @role='cousin')]">
			<xsl:value-of select="$newline"/>
			<dl class="prereqlinks">
				...
				<xsl:apply-templates mode="prereqs"
					select="descendant::*[generate-id(.)=generate-id(key('link',concat(ancestor::*[contains(@class, ' topic/related-links ')]/parent::*[contains(@class, ' topic/topic ')]/@id, ' ', @href,@type,@role,@platform,@audience,@importance,@outputclass,@keyref,@scope,@format,@otherrole,@product,@otherprops,@rev,@class,child::*))[1])]
     [@href]
	...
	<xsl:key name="omit-from-unordered-links"
		match="*[@importance='required' and (not(@role) or @role='sibling' or @role='friend' or @role='cousin')]"
		use="1"/>
	...
	<xsl:template match="*[contains(@class,' topic/related-links ')]" name="topic.related-links">
		<div>
			...
			<xsl:if
				test="contains($include.roles, ' next ') or contains($include.roles, ' previous ') or contains($include.roles, ' parent ')">
...
-->
	<xsl:template name="ol-child-links">
		<xsl:if
			test="descendant::*[contains(@class, ' topic/link ')][@role='child' or @role='descendant'][parent::*/@collection-type='sequence'][not(ancestor::*[contains(@class, ' topic/linklist ')])]">
			<xsl:value-of select="$newline"/>
	...
	<xsl:template match="*[contains(@class, ' topic/link ')][@type='topic']"
		mode="related-links:get-group-priority" name="related-links:group-priority.topic"
		priority="2">
		<xsl:call-template name="related-links:group-priority."/>
	</xsl:template>
	<xsl:template match="*[contains(@class, ' topic/link ')][@type='topic']"
		mode="related-links:get-group" name="related-links:group.topic" priority="2">
		<xsl:call-template name="related-links:group."/>
	</xsl:template>
	<xsl:template match="*[contains(@class, ' topic/link ')][@type='topic']"
		mode="related-links:result-group" name="related-links:group-result.topic" priority="2">
		<xsl:param name="links"/>
	...
	<xsl:template match="*[contains(@class, ' topic/link ')][@role='next' or @role='previous']"
		mode="breadcrumb">
	...
	<xsl:template name="nextlink" match="*[contains(@class, ' topic/link ')][@role='next']"
		priority="2">
		<strong>
			<!-- Allow for unknown metadata (future-proofing) -->
			<xsl:apply-templates
	...
	<xsl:template name="prevlink" match="*[contains(@class, ' topic/link ')][@role='previous']"
		priority="2">
		<strong>
			<!-- Allow for unknown metadata (future-proofing) -->
			<xsl:apply-templates
	...
	<xsl:template match="*[contains(@class, ' topic/link ')][@role='child' or @role='descendant']"
	...
	<xsl:template
		...
		priority="3" name="topic.link_orderedchild">
		<xsl:variable name="el-name">
			<xsl:choose>
				<xsl:when test="contains(../@class,' topic/linklist ')">div</xsl:when>
				<xsl:otherwise>li</xsl:otherwise>
	...
	<xsl:template match="*[contains(@class, ' topic/link ')]" name="topic.link">
		<xsl:if
			test="(@role and contains($include.roles, concat(' ', @role, ' '))) or
			...
			<xsl:choose>
				...
				<xsl:when
					test="@role and not(@role='cousin' or @role='external' or @role='friend' or @role='other' or @role='sample' or @role='sibling')">
					<div>
						<xsl:call-template name="makelink"/>
					</div>
					<xsl:value-of select="$newline"/>
				...
				<xsl:when
					test="(key('hideduplicates', concat(ancestor::*[contains(@class, ' topic/related-links ')]/parent::*[contains(@class, ' topic/topic ')]/@id, ' ',@href,@scope,@audience,@platform,@product,@otherprops,@rev,@type,normalize-space(child::*))))[2]">
					<xsl:choose>
						<xsl:when
							test="generate-id(.)=generate-id((key('hideduplicates', concat(ancestor::*[contains(@class, ' topic/related-links ')]/parent::*[contains(@class, ' topic/topic ')]/@id, ' ',@href,@scope,@audience,@platform,@product,@otherprops,@rev,@type,normalize-space(child::*))))[1])">
							<div>
								<xsl:call-template name="makelink"/>
							</div>
							<xsl:value-of select="$newline"/>
	...
	<xsl:template match="*[contains(@class, ' topic/link ')]/*[contains(@class, ' topic/desc ')]"
		name="topic.link_desc">
		<xsl:apply-templates select="*|text()"/>
	</xsl:template>
	...
	<xsl:template match="*[contains(@class,' topic/linklist ')]" name="topic.linklist">
		...
		<xsl:choose>
			...
			<xsl:when
				test="parent::*[contains(@class,' topic/related-links ')] and not(child::*[contains(@class,' topic/link ')][@role='child' or @role='descendant'])">
				<div class="linklist">
			...
			<xsl:when
				test="child::*[contains(@class,' topic/link ')][@role='child' or @role='descendant']">
	...
	<xsl:key name="omit-from-unordered-links"
		match="*[ancestor::*[contains(@class,' topic/linklist ')]]" use="1"/>
	...
	<xsl:template match="*" mode="processlinklist">
		...
		<xsl:apply-templates select="*[contains(@class,' topic/desc ')]"/>
		<xsl:for-each
			select="*[contains(@class,' topic/linklist ')]|*[contains(@class,' topic/link ')]">
			<xsl:choose>
				<!-- for children, div wrapper is created in main template -->
				<xsl:when
					test="contains(@class,' topic/link ') and (@role='child' or @role='descendant')">
					<xsl:value-of select="$newline"/>
					<xsl:apply-templates select="."/>
				</xsl:when>
	...
	<xsl:template name="linkdupinfo">
		<xsl:if
			test="(key('linkdup', concat(ancestor::*[contains(@class, ' topic/related-links ')]/parent::*[contains(@class, ' topic/topic ')]/@id, ' ', @href)))[2]">
			<xsl:if
				test="generate-id(.)=generate-id((key('linkdup', concat(ancestor::*[contains(@class, ' topic/related-links ')]/parent::*[contains(@class, ' topic/topic ')]/@id, ' ', @href)))[1])">
				<!-- If the link is exactly the same, do not output message.  The duplicate will automatically be removed. -->
				<xsl:if
					test="not(key('link', concat(ancestor::*[contains(@class, ' topic/related-links ')]/parent::*[contains(@class, ' topic/topic ')]/@id, ' ', @href,@type,@role,@platform,@audience,@importance,@outputclass,@keyref,@scope,@format,@otherrole,@product,@otherprops,@rev,@class,child::*))[2])">
	...
	<xsl:template match="*" mode="add-href-attribute">
		<xsl:if test="@href and normalize-space(@href)!=''">
			<xsl:attribute name="href">
				<xsl:apply-templates
					select="." mode="determine-final-href"/></xsl:attribute>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="*" mode="ditamsg:link-may-be-duplicate">
		...
		<xsl:call-template name="output-message">
			...
			<xsl:with-param name="msgparams">%1=<xsl:value-of select="$href"/>;%2=<xsl:value-of
					select="$outfile"/></xsl:with-param>
		</xsl:call-template>
	</xsl:template>
